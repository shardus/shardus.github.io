...
let UniswapConvertWidget = async function (config) {
  ...
  try {
    G.web3.eth.currentProvider.setMaxListeners(1000)
  } catch (e) {
    console.warn('Unable to set max listener for web3.')
  }
  ...
  function getSumamry (tokenAddress) {
    ...
    if (foundToken) {
      const foundSummary = G.summaryList.find(s => s.token_id === foundToken.id)
      return foundSummary
    }
  }
